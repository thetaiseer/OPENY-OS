...
(function liquidGlassKit() {
  ...
  var GlassToast = {
    ...
    show: function (type, title, desc, duration) {
      ...
      /* Build toast DOM with textContent to avoid XSS */
      var iconSpan = document.createElement("span");
      iconSpan.className = "glass-toast__icon";
      iconSpan.setAttribute("aria-hidden", "true");
      iconSpan.textContent = GlassToast.ICONS[type] || "";

      var body = document.createElement("div");
      body.className = "glass-toast__body";

      var titleEl = document.createElement("strong");
      titleEl.className = "glass-toast__title";
      titleEl.textContent = title || GlassToast.LABELS[type] || "";
      body.appendChild(titleEl);

      if (desc) {
        var descEl = document.createElement("p");
        descEl.className = "glass-toast__desc";
  ...
  var GlassChips = {
    init: function () {
      ...
      function commitChip() {
        ...
        chipLabel.className = "glass-chip__label";
        chipLabel.textContent = label;

        var chipDismiss = document.createElement("button");
        chipDismiss.className = "glass-chip__dismiss";
        chipDismiss.setAttribute("aria-label", "Remove " + label);
        chipDismiss.textContent = "\u2715";

        chip.appendChild(chipLabel);
        chip.appendChild(chipDismiss);
      ...
      function openInput() {
        ...
        chipInput.className = "glass-chip-input";
        chipInput.type = "text";
        chipInput.placeholder = "Add chip\u2026";
        chipInput.setAttribute("aria-label", "New chip label");

        var confirmBtn = document.createElement("button");
        confirmBtn.className = "glass-chip-input-confirm";
  ...
  GlassTable.prototype._applySort = function () {
    ...
    /* Stagger re-entry — reset animations in one pass, then apply in another
       to avoid per-row layout thrashing from offsetHeight reads. */
    this._filtered.forEach(function (row) {
      row.el.style.animation = "none";
    });
    /* Single forced reflow to flush all animation resets at once */
    void this.tbody.offsetHeight;
