  ...
  useEffect(() => {
    ...
    void (async () => {
      const [tk, ast, ct, rtk, rast, rct] = await Promise.allSettled([
        ...
        supabase.from('tasks').select('id,title,status,priority,due_date').eq('client_id', clientId)
          .not('status', 'in', '("done","completed","delivered","cancelled")').order('due_date', { ascending: true }).limit(4),
        supabase.from('assets').select('id,name,file_type,created_at,thumbnail_url,preview_url,file_url').eq('client_id', clientId)
          .order('created_at', { ascending: false }).limit(4),
        supabase.from('content_items').select('id,title,status,created_at').eq('client_id', clientId)
          .order('created_at', { ascending: false }).limit(4),
      ]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {loading ? (
          ...
          [
            { label: t('tasks'),   value: counts.tasks,   icon: <CheckSquare size={16} />, href: `/clients/${slug}/tasks`   },
            { label: t('assets'),  value: counts.assets,  icon: <FolderOpen  size={16} />, href: `/clients/${slug}/assets`  },
            { label: t('content'), value: counts.content, icon: <FileText    size={16} />, href: `/clients/${slug}/content` },
          ].map(({ label, value, icon, href }) => (
            <Link key={label} href={href}
              className="rounded-2xl border p-5 text-center hover:opacity-80 transition-opacity"
              style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
        ...
        <div className="rounded-2xl border p-5" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Active Tasks</h3>
            </div>
            <Link href={`/clients/${slug}/tasks`} className="text-xs hover:opacity-70 transition-opacity" style={{ color: 'var(--accent)' }}>
              View all
            </Link>
          </div>
          <div className="space-y-2">
            {recentTasks.map(task => {
              ...
              return (
                <div key={task.id} className="flex items-center justify-between gap-3 rounded-xl px-3 py-2"
                  style={{ background: 'var(--surface-2)' }}>
                  <div className="flex items-center gap-2 min-w-0">
                    {isOverdue
                      ? <AlertCircle size={12} className="shrink-0" style={{ color: '#ef4444' }} />
                      : <Clock size={12} className="shrink-0" style={{ color: 'var(--text-secondary)' }} />}
                    <p className="text-sm font-medium truncate" style={{ color: 'var(--text)' }}>{task.title}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {task.due_date && (
                      <span className="text-xs" style={{ color: isOverdue ? '#ef4444' : 'var(--text-secondary)' }}>
                        {fmtDate(task.due_date)}
                      </span>
                    )}
                    <span className="text-[10px] px-1.5 py-0.5 rounded font-medium"
                      style={{ background: `${taskStatusColor(task.status)}20`, color: taskStatusColor(task.status) }}>
        ...
        <div className="rounded-2xl border p-5" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Recent Assets</h3>
            </div>
            <Link href={`/clients/${slug}/assets`} className="text-xs hover:opacity-70 transition-opacity" style={{ color: 'var(--accent)' }}>
              View all
            </Link>
          </div>
        ...
        <div className="rounded-2xl border p-5" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Recent Content</h3>
            </div>
            <Link href={`/clients/${slug}/content`} className="text-xs hover:opacity-70 transition-opacity" style={{ color: 'var(--accent)' }}>
              View all
            </Link>
          </div>
          <div className="space-y-2">
            {recentContent.map(item => (
              <div key={item.id} className="flex items-center justify-between gap-3 rounded-xl px-3 py-2"
                style={{ background: 'var(--surface-2)' }}>
