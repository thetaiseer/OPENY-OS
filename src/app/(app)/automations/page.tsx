...
export default function AutomationsPage() {
  ...
  const [form, setForm] = useState({ name: '', description: '', trigger_type: '', is_active: true });
  ...
  const openEdit = (rule: AutomationRule) => {
    setEditRule(rule);
    setForm({ name: rule.name, description: rule.description ?? '', trigger_type: rule.trigger_type, is_active: rule.is_active });
    setSaveErr(null);
    setModalOpen(true);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: 'var(--text)' }}>Automations</h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          <Zap size={40} className="mx-auto mb-3 opacity-30" style={{ color: 'var(--text-secondary)' }} />
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>No automations yet. Create your first rule!</p>
          <p className="text-xs mt-2 max-w-sm mx-auto" style={{ color: 'var(--text-secondary)' }}>
        ...
        <div className="space-y-3">
          {rules.map(rule => (
            ...
            >
              ...
              >
                <Zap size={16} style={{ color: rule.is_active ? 'var(--accent)' : 'var(--text-secondary)' }} />
              </div>

              {/* Info */}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold" style={{ color: 'var(--text)' }}>{rule.name}</p>
                <p className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                  Trigger: {triggerLabel(rule.trigger_type)}
                  {rule.run_count > 0 && <span className="ml-2">· Ran {rule.run_count}×</span>}
                </p>
                {rule.description && (
                  <p className="text-xs mt-0.5 truncate" style={{ color: 'var(--text-secondary)' }}>{rule.description}</p>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg hover:bg-[var(--surface-2)] transition-colors text-red-500"
                >
                  <Trash2 size={14} />
                </button>
              </div>
      ...
      {modalOpen && (
        <div className="openy-modal-overlay fixed inset-0 z-50 flex items-start sm:items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="openy-modal-panel w-full max-w-md rounded-2xl p-6 space-y-4 my-auto max-h-[calc(100dvh-1.5rem)] sm:max-h-[calc(100dvh-2rem)] overflow-y-auto">
            ...
            <form onSubmit={e => void handleSave(e)} className="space-y-3">
              ...
              <div>
                <label className="text-sm font-medium block mb-1" style={{ color: 'var(--text)' }}>Trigger *</label>
                <select
                  required
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="rounded"
                />
                <span className="text-sm" style={{ color: 'var(--text)' }}>Active (run this rule)</span>
              </label>
