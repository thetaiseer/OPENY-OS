...
const VALID_MAIN_CATEGORIES: string[] = MAIN_CATEGORIES.map((category) => category.slug);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Supabase configuration error' },
      { status: 500 },
    );
  }
  ...
  if (subCategory) {
    const validSubs = (SUBCATEGORIES[mainCategory as MainCategorySlug] ?? []).map(
  ...
  if (fileSize > multipartThreshold) {
    return NextResponse.json(
      {
        error: `This file is ${(fileSize / (1024 * 1024)).toFixed(
          1,
        )} MB. Use multipart presigned upload for files over ${(
          multipartThreshold /
          (1024 * 1024)
        ).toFixed(0)} MB.`,
        code: 'USE_MULTIPART',
