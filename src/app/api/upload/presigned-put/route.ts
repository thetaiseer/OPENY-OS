...
const BLOCKED_EXTENSIONS = new Set(['exe','bat','cmd','sh','bash','ps1','msi','vbs','php','py','rb','pl','cgi','app','com','scr','pif','reg','dll','so']);
...
function sanitizeFileName(name: string): string { return name.replace(/\s+/g, '_').replace(/[^a-zA-Z0-9._\-]/g, ''); }
...
export async function POST(req: NextRequest) {
  ...
  if (!rl.allowed) {
    return NextResponse.json({ error: 'Upload limit exceeded. Please try again later.' }, { status: 429, headers: { 'Retry-After': String(Math.ceil((rl.resetAt - Date.now()) / 1000)) } });
  }
  ...
  try { supabase = getServiceClient(); } catch (err: unknown) { return NextResponse.json({ error: err instanceof Error ? err.message : 'Supabase configuration error' }, { status: 500 }); }

  const { workspaceId, error: workspaceError } = await resolveWorkspaceForRequest(req, supabase, auth.profile.id);
  if (!workspaceId) return NextResponse.json({ error: workspaceError ?? 'Unable to resolve workspace from session' }, { status: 403 });

  clientName = await resolveUploadClientDisplayName(supabase, workspaceId, clientName, clientId);
  if (!clientName) return fail('clientName is required, or pass a valid clientId in this workspace so the name can be resolved.');
  if (!VALID_MAIN_CATEGORIES.includes(mainCategory)) return fail(`Invalid mainCategory. Must be one of: ${VALID_MAIN_CATEGORIES.join(', ')}`);

  if (subCategory) {
    const validSubs = (SUBCATEGORIES[mainCategory as MainCategorySlug] ?? []).map((s) => s.slug);
    if (validSubs.length > 0 && !validSubs.includes(subCategory)) return fail(`Invalid subCategory "${subCategory}" for mainCategory "${mainCategory}"`);
  }

  const ext = getExtension(fileName);
  if (BLOCKED_EXTENSIONS.has(ext)) return fail('File type not allowed: executable and script files are blocked');
  if (!fileSize || fileSize <= 0) return fail('fileSize must be a positive number');

  const maxBytes = getMaxUploadBytes();
  if (maxBytes > 0 && fileSize > maxBytes) return NextResponse.json({ error: uploadSizeExceededMessage(maxBytes) }, { status: 413 });

  const multipartThreshold = getMultipartThresholdBytes();
  if (fileSize > multipartThreshold) {
    return NextResponse.json({ error: `This file is ${(fileSize / (1024 * 1024)).toFixed(1)} MB. Use multipart presigned upload for files over ${(multipartThreshold / (1024 * 1024)).toFixed(0)} MB.`, code: 'USE_MULTIPART' }, { status: 413 });
  }
  ...
  if (customName) {
    ...
    displayName = dotExt && base.toLowerCase().endsWith(dotExt.toLowerCase()) ? base : `${base}${dotExt}`;
  }

  let storageKey: string;
  try {
    ({ storageKey } = await allocateWorkspaceAssetStorageKey(workspaceId, clientId, monthKey, originalName, mainCategory, subCategory));
  } catch (e) {
  ...
  try {
    ...
    console.error('[upload/presigned-put] failure', { provider: 'r2', bucketName, storageKey, error: msg });
