...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [clientsResult, tasksResult, assetsResult, schedulesResult, approvalsResult, teamMembersResult] =
      await Promise.allSettled([
        ...
        db.from('team_members').select('id, full_name, email, permission_role, status, profile_id').eq('status', 'active'),
      ]);
    ...
    const teamMembers = teamMembersResult.status === 'fulfilled' ? (teamMembersResult.value.data ?? []) : [];
