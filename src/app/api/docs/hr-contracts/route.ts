...
export async function POST(req: NextRequest) {
  ...
  const { contract_number, employee_full_name } = body as { contract_number?: string; employee_full_name?: string };
  if (!contract_number?.trim())    return NextResponse.json({ error: 'contract_number is required' }, { status: 400 });
  if (!employee_full_name?.trim()) return NextResponse.json({ error: 'employee_full_name is required' }, { status: 400 });
