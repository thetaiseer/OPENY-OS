...
export async function PATCH(req: NextRequest, { params }: { params: Promise<Params> }) {
  ...
  if (nextNum) {
    ...
    if (nextNum !== prevNum) {
      const unique = await dbDocumentNumberIsUnique(db, 'docs_hr_contracts', nextNum, id);
      if (!unique) {
        return NextResponse.json(
          {
            error:
              'This contract number is already used by another HR contract. Choose a different number.',
