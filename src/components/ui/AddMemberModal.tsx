...
export function AddMemberModal({ open, onClose }: AddMemberModalProps) {
  ...
  return (
    <Modal open={open} onClose={handleClose} title={isAr ? "إضافة عضو جديد" : "Add team member"}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        />
        <Field
          label={isAr ? "الدور / القسم" : "Role / Department"}
          required
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          >
            <UserPlus size={16} />
            {loading ? (isAr ? "جارٍ الإضافة…" : "Adding…") : (isAr ? "إضافة عضو" : "Add member")}
          </button>
        </div>
      </form>
    </Modal>
