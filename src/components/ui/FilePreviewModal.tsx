...
function isImage(name: string, mime?: string | null): boolean {
  return (
    /\.(jpe?g|png|webp|gif|svg|bmp|avif)$/i.test(name) || (!!mime && mime.startsWith('image/'))
  );
}
...
function isText(name: string, mime?: string | null): boolean {
  return (
    /\.(txt|md|log|ini|yaml|yml|toml|xml|html|htm|css|js|ts|sh|py|rb|java|go|rs|c|cpp|h|php)$/i.test(
      name,
    ) ||
...
function ImagePreview({ src, alt }: { src: string; alt: string }) {
  ...
  if (failed) {
    return (
      <div className="flex flex-col items-center gap-3 py-12">
        ...
        <p className="text-sm text-white/60">Image could not be loaded</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full flex-col items-center gap-3">
      <div
        className="relative flex max-h-[80vh] w-full items-center justify-center overflow-auto rounded-xl"
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={reset}
          title="Reset zoom"
          className="flex h-8 items-center justify-center rounded-lg bg-white/10 px-3 text-xs font-medium text-white transition-colors hover:bg-white/20"
...
function TextPreview({ src, name }: { src: string; name: string }) {
  ...
  if (error || content === null) {
    return (
      <div className="flex flex-col items-center gap-3 py-12">
        ...
        <p className="text-sm text-white/60">Could not load file content</p>
      </div>
    );
  }
  ...
  if (csv) {
    ...
    return (
      <div
        className="max-h-[80vh] w-full overflow-auto rounded-xl shadow-2xl"
        ...
        >
          <thead>
            {rows[0] && (
              ...
              >
                {rows[0].map((cell, i) => (
                  <th key={`h-${i}`} className="whitespace-nowrap px-3 py-2 font-semibold">
                    {cell.replace(/^"|"$/g, '')}
                  </th>
                ))}
              </tr>
  ...
  return (
    ...
    >
      <pre
        className="whitespace-pre-wrap break-words p-4 text-xs leading-relaxed"
        style={{
...
export default function FilePreviewModal({ file, onClose }: FilePreviewModalProps) {
  ...
  return (
    ...
    >
      ...
      {/* ── Preview body ── */}
      <div className="flex min-h-0 flex-1 flex-col items-center justify-center overflow-auto p-5">
