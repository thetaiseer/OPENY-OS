      ...
      toolbar={(
        <div className="docs-workspace-quickbar">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => {
                ...
                window.open(`/api/docs/hr-contracts/${editingId}/export`, '_blank', 'noopener,noreferrer');
          ...
          <div className="docs-workspace-quickbar-grid">
            ...
            <div><label>History</label><select className={inp} value={editingId ?? ''} onChange={e => { const selected = contracts.find(c => c.id === e.target.value); if (selected) loadIntoForm(selected); else resetForm(); }}><option value="">New contract</option>{contracts.map(c => <option key={c.id} value={c.id}>{c.contract_number} · {c.employee_full_name}</option>)}</select></div>
          </div>
        </div>
      )}
      editor={(
