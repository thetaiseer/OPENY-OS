...
type SF = {
  ...
  sig_company_rep: string;
  sig_employee_name: string;
  sig_date: string;
  sig_place: string;
};
...
function HistoryPanel({
  ...
  const visible = contracts.filter((c) => {
    ...
    if (
      ...
      !c.employee_full_name.toLowerCase().includes(search.toLowerCase())
    )
      return false;
    return true;
  });
...
export default function HrContractPage() {
  ...
  function setField<K extends keyof SF>(k: K, v: SF[K]) {
    setForm((f) => ({ ...f, [k]: v }));
  }

  function resetForm() {
    setEditingId(null);
    setForm(blank(nextNum(contracts)));
    setError('');
    setActiveTab('editor');
  }
  ...
  function loadIntoForm(c: DocsHrContract) {
    ...
    setForm({
      ...
      employee_full_name: c.employee_full_name,
      employee_national_id: c.employee_national_id ?? '',
      employee_address: c.employee_address ?? '',
      employee_phone: c.employee_phone ?? '',
      employee_email: c.employee_email ?? '',
      employee_nationality: c.employee_nationality ?? '',
      employee_marital_status: c.employee_marital_status ?? 'Single',
      job_title: c.job_title ?? '',
      department: c.department ?? '',
      direct_manager: c.direct_manager ?? '',
      employment_type: c.employment_type ?? 'full_time',
      start_date: c.start_date ?? today(),
      contract_duration: c.contract_duration ?? '1 year',
      probation_period: c.probation_period ?? '3 months',
      workplace: c.workplace ?? '',
      salary: c.salary,
      payment_method: c.payment_method ?? 'Bank Transfer',
      payment_date: c.payment_date ?? 'End of month',
      benefits: c.benefits,
      daily_hours: c.daily_hours,
      work_days: c.work_days ?? 'Sunday – Thursday',
      annual_leave: c.annual_leave,
      legal_clauses: c.legal_clauses,
      sig_company_rep: c.sig_company_rep ?? '',
      sig_employee_name: c.sig_employee_name ?? '',
  ...
  async function handleBackup() {
    ...
    await fetch('/api/docs/backups', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ module: 'hr_contracts', data: contracts, label }),
    });
  }

  async function handleClearAll() {
    await Promise.all(
      contracts.map((c) => fetch(`/api/docs/hr-contracts/${c.id}`, { method: 'DELETE' })),
    );
    await load();
    resetForm();
  }
  ...
  async function handleRestoreData(data: unknown) {
    ...
    for (const item of data as DocsHrContract[]) {
      ...
      const res = await fetch('/api/docs/hr-contracts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
